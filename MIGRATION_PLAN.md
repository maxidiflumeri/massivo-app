...
- [ ] **5.A.2** — `ContactsService` org-scope: CRUD por `organizationId`, búsqueda multi-clave (cascada `externalId > dni > cuit > email > phoneE164`), endpoints `GET /api/contacts`, `GET /api/contacts/:id`, `POST /api/contacts`, `PATCH /api/contacts/:id`, `DELETE /api/contacts/:id`. Validación AR: DNI (7-8 dígitos) + CUIT (checksum mod-11).
- [ ] **5.A.3** — CSV import wizard: `POST /api/contacts/import` con multipart (CSV + mapping JSON). Worker procesa con cascada de identidad: strong key (externalId/dni/cuit) → auto-upsert; weak key (email/phone) → si conflicto crear `ContactMergeSuggestion`. Endpoints `GET /api/contacts/imports` (lista jobs) + `GET /api/contacts/imports/:id` (detalle + counters + errors). `ContactImportJob.mapping` guarda el mapeo elegido (sin presets per-org en V1).
- [ ] **5.A.4** — Merge UI: `GET /api/contacts/merge-suggestions` (PENDING) + `POST /api/contacts/merge-suggestions/:id/accept|reject`. Accept: merge `right` → `left` (relink `EmailContact`/`WapiContact`/`ContactTag`/`ContactListMember`, delete `right`). Frontend en `5.D`.
