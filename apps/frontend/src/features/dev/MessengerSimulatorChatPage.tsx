...
const STORAGE_KEY = 'massivo:dev-messenger-chat:state';
...
/**
 * Página dev `/dashboard/dev/channels/messenger/chat` (Fase 2). Sandbox de Messenger:
 * el "cliente virtual" (PSID) inyecta inbounds Messenger-shaped vía
 * `/api/dev/channels/messenger/inbound`; el bot responde (en modo test, sin pegar a
 * Meta) y se ve acá + en el inbox real (badge Messenger). Requiere un Channel
 * MESSENGER de test, que se crea con "Conectar canal" (opcionalmente atado a un bot).
 */
export function MessengerSimulatorChatPage() {
  ...
  const [psid, setPsid] = useState(persisted.psid ?? 'user-1');
  ...
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ botId, channelId, psid }));
    } catch {
      // no-op
    }
  }, [botId, channelId, psid]);
  ...
  const ensureChannel = useCallback(async () => {
    ...
    try {
      const res = await api.post<{ id: string; pageId: string; botId: string | null }>(
        '/api/dev/channels/messenger/ensure',
        botId ? { botId } : {},
      );
      setChannelId(res.id);
      notify.success(`Canal Messenger listo (page ${res.pageId})`);
    } catch (e) {
  ...
  async function sendInbound(body: { text?: string; quickReplyPayload?: string }) {
    if (!channelId || !psid.trim()) {
      notify.error('Primero conectá un canal y poné un PSID');
      return;
    }
    try {
      await api.post('/api/dev/channels/messenger/inbound', {
        channelId,
        psid: psid.trim(),
        ...body,
      });
  ...
  return (
    <Box sx={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
      <Paper sx={{ p: 1.5 }}>
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} alignItems={{ md: 'center' }}>
          <Stack direction="row" spacing={1} alignItems="center" sx={{ minWidth: 0 }}>
            <ChatIcon sx={{ color: '#0084FF' }} />
            <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>
              Chat simulado Messenger (dev)
            </Typography>
          </Stack>
          <Box sx={{ flex: 1 }} />
          <Select
          ...
          <Tooltip title="Crea (o reusa) un canal Messenger de test y lo conecta al bot elegido">
            <span>
              <Button
                size="small"
                variant="outlined"
        ...
        {!channelId && (
          <Alert severity="info" sx={{ mt: 1.5 }}>
            Elegí un bot (publicado) y tocá <b>Conectar canal</b> para crear un canal Messenger de
            test. Después escribí como cliente abajo: el bot responde y la conversación aparece en el
            inbox con el badge de Messenger.
          </Alert>
        )}
        {channelId && (
      ...
      <Paper sx={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        >
          <Avatar sx={{ bgcolor: '#0084FF', width: 36, height: 36 }}>
          ...
          <Chip size="small" label="Inbound simulado" sx={{ bgcolor: '#0084FF22' }} />
        </Stack>
        {!ready ? (
          <EmptyHint text="Conectá un canal y poné un PSID para empezar." />
        ) : (
          <ClientThread
            messages={messages}
            loading={loading}
