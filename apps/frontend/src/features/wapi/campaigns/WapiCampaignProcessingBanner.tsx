...
import type { WapiCampaignReport, WapiCampaignStatus } from './types';

interface Props {
  campaignId: string;
  totalReports: number;
  report: WapiCampaignReport | null;
  socket: Socket | null;
  status: Extract<WapiCampaignStatus, 'PROCESSING' | 'PAUSED'>;
...
type LogStatus = 'SENT' | 'FAILED' | 'CANCELED';
...
export function WapiCampaignProcessingBanner({
  campaignId,
  totalReports,
  report,
  socket,
  ...
  const totalCounted = pending + sent + delivered + read + failed + canceled;
  ...
  useEffect(() => {
    ...
    socket.on('wapi.report.log', handler);
    return () => {
      socket.off('wapi.report.log', handler);
    };
  }, [socket, campaignId]);
  ...
  return (
    <Paper sx={{ p: 3 }}>
      ...
      <Stack direction="row" spacing={1} sx={{ mt: 1.5, flexWrap: 'wrap' }} useFlexGap>
        ...
        {delivered > 0 && (
          <Chip size="small" label={`Entregados: ${delivered}`} color="info" variant="outlined" />
        )}
        {read > 0 && <Chip size="small" label={`Leídos: ${read}`} color="info" />}
        {failed > 0 && <Chip size="small" label={`Fallidos: ${failed}`} color="error" />}
        {canceled > 0 && (
          <Chip size="small" label={`Cancelados: ${canceled}`} variant="outlined" />
        )}
      </Stack>
    ...
    >
      {entries.map((e) => (
        <Box key={`${e.ts}-${e.reportId}`} sx={{ whiteSpace: 'nowrap' }}>
          ...
          <Box component="span">{e.phone}</Box>
          {e.metaMessageId && (
            <Box component="span" sx={{ color: 'grey.500' }}>
              {' '}
              · wamid={truncate(e.metaMessageId, 28)}
            </Box>
          )}
          {e.error && (
