...
- **Fase actual:** Fase 4 — Canal WhatsApp Cloud API (**sub-A ✅, sub-B ✅, sub-C ✅, sub-D ✅, sub-E ✅, sub-F.1 ✅, sub-F.1.a ✅, sub-F.1.b ✅, sub-F.1.c ✅ (mapping vars), sub-F.2.a ✅, sub-F.2.b ✅, sub-F.2.c ✅ (markdown + dark mode preview)**; siguen **4.F.2.d media upload Meta** (renombrado de 4.F.2.c original), 4.F.3-4 inbox, 4.G snippets, 4.H opt-out, 4.I welcome, 4.J live dashboard, 4.K botones)
- **Fases completadas:** Fase 0 ✅ + Fase 1 ✅ + Fase 2 ✅ + **Fase 3 ✅** (3.E inbound postergado, decisión del dueño)
- **Última actualización:** 2026-05-04
- **Branch principal:** `main`
- **Último commit (próximo a este cierre):** Sesión 23 — fixes y mejoras de UX sobre 4.F.1/4.F.2 detectados en smoke test del dueño. (1) **4.F.2.c renderizado markdown WhatsApp en preview** (`*bold*`, `_italic_`, `~strike~`, `` `mono` ``, ` ```block``` `) + fix dark mode de las burbujas. (2) **4.F.1.c mapeo CSV → vars del template**: sección de mapping en `WapiCampaignDetailPage` con dropdown por cada `{{N}}`, alimentada por el CSV en pantalla + nuevo endpoint `GET /api/wapi/campaigns/:id/contacts/data-keys` que extrae las keys ya cargadas. Persistido en `campaign.config.bodyVars`, exigido por `canSend`. Fixes Meta #132000 (vars no llegaban) y #131008 (vars vacías). Parser CSV ahora duplica `name`/`nombre` en `contact.data` para que sean referenciables; worker tiene fallback a `contact.name` para contactos legacy.
