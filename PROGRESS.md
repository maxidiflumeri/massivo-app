...
- **Fase actual:** Fase 4 — Canal WhatsApp Cloud API (**sub-A ✅, sub-B ✅, sub-C ✅, sub-D ✅, sub-E ✅**; siguen 4.F inbox, 4.G snippets, 4.H opt-out, 4.I welcome, 4.J live dashboard, 4.K botones)
- **Fases completadas:** Fase 0 ✅ + Fase 1 ✅ + Fase 2 ✅ + **Fase 3 ✅** (3.E inbound postergado, decisión del dueño)
- **Última actualización:** 2026-05-04
- **Branch principal:** `main`
- **Último commit (próximo a este cierre):** Sesión 21 cierre (4.E CRUD completo de campañas WAPI). Migration `add_canceled_to_wapi_report_status` (enum CANCELED). DTOs nuevos (phone E.164 lenient, ArrayMaxSize 5000). `WapiCampaignsService` extendido con `create/findAll/findOne/update/remove/addContacts/pause/resume/forceClose/listReports/getReport`. `forceClose` ahora marca `CANCELED` (no FAILED) → funnel limpio. `WapiWorkerService` con early-exit si report.status≠PENDING (fix de bug latente: jobs huérfanos post-forceClose enviaban igual). Endpoints completos en `WapiCampaignsController`. 24 tests nuevos en service spec + 1 en worker spec. Backend full **325/325 ✅**. Para retomar: arrancar **4.F — UI frontend (admin de campañas WAPI + Inbox conversacional)**. Ver detalle más abajo.
