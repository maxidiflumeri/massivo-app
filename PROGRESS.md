...
- **Fase actual:** Fase 5 — Contacts unificados + Timeline cross-canal (**5.A.1 ✅** schema org-scope + identity fields + ContactMergeSuggestion + ContactImportJob + migración con backfill idempotente + tenant-models + CASL). Pendiente: 5.A.2 (CRUD service + endpoints), 5.A.3 (CSV import wizard), 5.A.4 (merge UI), 5.B (timeline aggregator), 5.C (búsqueda), 5.D (frontend), 5.E (reportes).
