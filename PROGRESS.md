...
- **Fase actual:** Fase 4 — Canal WhatsApp Cloud API (**sub-A ✅, sub-B ✅, sub-C ✅, sub-D ✅, sub-E ✅, sub-F.1 ✅, sub-F.1.a ✅, sub-F.1.b ✅**; siguen 4.F.2 templates Massivo→Meta, 4.F.3-4 inbox, 4.G snippets, 4.H opt-out, 4.I welcome, 4.J live dashboard, 4.K botones)
- **Fases completadas:** Fase 0 ✅ + Fase 1 ✅ + Fase 2 ✅ + **Fase 3 ✅** (3.E inbound postergado, decisión del dueño)
- **Última actualización:** 2026-05-04
- **Branch principal:** `main`
- **Último commit (próximo a este cierre):** Sesión 22 cierre extendido (4.F.1 + 4.F.1.a + 4.F.1.b: frontend WAPI completo de admin operativo). Sobre 4.F.1 (campañas) se sumó 4.F.1.a (`WapiConfigsPage` — CRUD de números/configs con secret fields opcionales en edit, switch isActive, modal único create/edit) y 4.F.1.b (`WapiTemplatesListPage` — catálogo read-only con sync dialog que dispara `POST /api/wapi/templates/sync/:configId` y muestra `WapiSyncSummary`, preview dialog con bubble estilo WhatsApp parsing components Meta). Routing y sidebar wireados: `/dashboard/wapi/configs` (entry "Números" con `DnsIcon`) y `/dashboard/wapi/templates` (entry "Templates" pasó de disabled a link). Type-check `@massivo/frontend` queda OK para los archivos nuevos. Para retomar: arrancar **4.F.2 — Templates en Massivo con posting a Meta** (todas las variantes: header text/image/video/document, body con `{{1}}…{{N}}`, footer, botones quick-reply/URL/phone, preview en vivo, AI suggestion como placeholder).
