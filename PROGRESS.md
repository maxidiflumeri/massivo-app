...
- **Fase actual:** Fase 5 — Contacts unificados + Timeline cross-canal (**5.A.1-4 ✅**, **5.B ✅** Timeline aggregator, **5.C ✅** Búsqueda avanzada, **5.D ✅** Frontend completo, **5.D.1 ✅** carga de contactos vía campañas con strong key obligatorio + Contact unificado). Pendiente: 5.E (reportes consolidados).
- **Fase 4:** WhatsApp Cloud API ✅ — (**sub-A ✅, sub-B ✅, sub-C ✅, sub-D ✅, sub-E ✅, sub-F.1 ✅, sub-F.1.a ✅, sub-F.1.b ✅, sub-F.1.c ✅ (mapping vars), sub-F.2.a ✅, sub-F.2.b ✅, sub-F.2.c ✅ (markdown + dark mode preview), sub-F.2.d ✅ (media upload Meta + storage local + render por tipo), sub-F.3 ✅ (inbox backend), sub-F.4 ✅ (inbox frontend), sub-G ✅ (quick replies admin), sub-H ✅ (opt-out por keyword + worker guard), sub-I ✅ (welcome message), sub-J ✅ (live dashboard WAPI), sub-P ✅ (webhook URL org-scoped + reveal verify token), sub-Q ✅ (throttle configurable por línea + override per-campaña), sub-R ✅ (scheduler de campañas WAPI + Email), sub-S ✅ (audit log infra + cobertura backend completa + endpoint `GET /api/audit-logs` + frontend `/dashboard/audit` con tabla/filtros/drawer), sub-K ✅ (botones interactivos: INBOX/BAJA/IGNORAR + payload variable), sub-L (MVP ✅ + ✅ chat ida-vuelta con `isTestMode`; pendiente virtual numbers), sub-L.1 ✅ (filtro inbox por línea), sub-M ✅ (`isTestMode` + chat simulado), sub-N ✅ (bot guiado por número con menús + handoff), sub-N.1 ✅ (editor visual react-flow + nodo MESSAGE encadenable), sub-N.2 ✅ (CAPTURE/MEDIA/CONDITION + interpolación `{{var}}`), sub-O.1 ✅ (multi-topic + router + feature flag env+per-org), sub-O.2 ✅ (UI multi-topic + router en el editor visual), sub-O.3 ✅ (Sandbox + Draft/Publish — Bloques 1-3 ✅: backend draft/publish + sandbox engine + UI con `SandboxDrawer` chat; falta smoke E2E manual), sub-O.4 ✅ (variables declarativas con defaults aplicados al iniciar sesión + panel CRUD + VarPicker en TextFields + Selects en CAPTURE.saveAs y CONDITION.var), sub-O.5 ✅ (nodo SET_VAR — asignación interna con coerción por tipo), sub-O.6 ✅ (suspensión bot + estado WAITING)**; pendiente **smoke E2E 4.O.3/4.O.6 con bot real, virtual numbers de 4.L**)
- **Fases completadas:** Fase 0 ✅ + Fase 1 ✅ + Fase 2 ✅ + **Fase 3 ✅** (3.E inbound postergado, decisión del dueño)
- **Última actualización:** 2026-05-07 (Sesión 52 — **5.D.1 Carga de contactos vía campañas con strong key obligatorio + Contact upsert**. Decisión de diseño tomada con el dueño: la carga de contactos siempre es **vía creación de campaña** (Email/WAPI) — no hay flujo standalone de import. Cada fila del CSV de campaña debe traer **al menos uno de `externalId` o `dni`**; eso garantiza que el `Contact` se cree o se mergee correctamente y que `EmailContact`/`WapiContact` queden linkeados al mismo Contact unificado cross-canal. Backend: nuevo `apps/backend/src/modules/contacts/contact-upsert.service.ts` (extrae la lógica de matching/merge de `ContactImportsService` a un servicio reusable; método público `upsert(input): Promise<{contactId, outcome: 'created'|'updated'|'suggested'}>`; cascada `findByStrongKey (externalId→dni→cuit)` → updateContact con catch P2002 + `maybeSuggestWeakConflict`; sino `findByWeakKey (email→phoneE164)` → conditional update o create+suggestion; sino `createContact`. Reusa los normalizers de `identity.ts`. Calls `requireContext()` para enforce TenantContext). `ContactsModule` exporta `ContactUpsertService` para que email/wapi modules lo inyecten via `imports: [ContactsModule]`. **EmailCampaignsService.addContacts** rewrite: collecta filas sin strong key → throw `BadRequestException` antes de tocar DB ("Cada fila debe traer externalId o dni. Filas inválidas: 1, 3, 7..."); per-row `contactUpsert.upsert(...)` + `prisma.scoped.emailContact.create({ contactId, ...})` (cambiado de `createMany` para poder attach `contactId` por fila); response shape extendido a `{ created, contactsCreated, contactsUpdated, suggestionsCreated }` (backwards-compat: `created` sigue siendo emailContact rows). DTO `CampaignContactDto` gana opcionales `externalId/dni/cuit/firstName/lastName` con `@IsString @MaxLength`. **WapiCampaignsService.addContacts** mirror del cambio email (`phone` en vez de `email`). Helper local `splitName('Juan Perez')` → `{firstName:'Juan', lastName:'Perez'}` para retro-compat con CSVs que mandan sólo `name`. **Borrado**: `ContactImportsService` + controller + DTO + spec; frontend `ContactsImportPage.tsx`; ruta `/dashboard/contacts/import` y botón "Importar CSV" del `ContactsListPage`; types `ContactImportJob`/`ContactImportJobPage`/`CreateImportRequest`. **Frontend campañas**: `CampaignDetailPage` (email) y `WapiCampaignDetailPage` (wapi) con `parseContactsCsv()` reescrito — reconoce headers `externalId|external_id|idexterno|dni|documento|cuit` además de email/phone/name, rechaza CSV sin strong-key column, valida fila-a-fila que tenga al menos uno de `externalId|dni`. Placeholders + copy actualizados ("Cada fila debe incluir externalId o dni"). `CampaignContactInput` types extendidos con los 5 campos opcionales. **Tests**: specs de campaña actualizados — mock `emailContact.create` (no createMany), nuevo mock `contactUpsert.upsert`, 4to constructor arg, tests "fila sin externalId/dni → BadRequest y no crea nada" + "PROCESSING NO permite addContacts → Conflict". 593/593 backend tests verde. Backend + frontend typecheck ✅. **Deferido**: el modelo Prisma `ContactImportJob` y el subject CASL `'ContactImportJob'` quedan huérfanos pero sin impacto (no hay migración, no hay code que los toque) — limpieza diferida a un housekeeping pass futuro. Próximo: smoke test E2E (subir CSV en una campaña con duplicados intencionales y verificar que los `Contact` se mergean por externalId, que aparecen sugerencias por email-conflict, y que el timeline cross-canal del Contact unificado muestra eventos de Email + WAPI). Después: 5.E (reportes consolidados). Sesión 51 anterior — **5.D Frontend Contacts (lista + ficha + timeline + merge UI + CSV import)**. Cierra Fase 5 en su parte UI. 4 páginas nuevas en `apps/frontend/src/features/contacts/`: `ContactsListPage`, `ContactDetailPage`, `MergeSuggestionsPage`, `ContactsImportPage` + `types.ts` con shapes mirror del backend. **5.D.1 Lista** `/dashboard/contacts`: tabla con name/email/phone/externalId/updatedAt; filtros `q + channel(email|wapi) + hasOpened/Clicked/Bounced (Switch)` + sort 6 combos (updatedAt|createdAt × ↑↓ + name A→Z|Z→A); botón "Aplicar" + "Limpiar"; cursor pagination con "Cargar más"; click-to-detail; header con botones "Sugerencias de merge" y "Importar CSV". Wire a `GET /api/contacts/search?q&channel&hasOpened&hasClicked&hasBounced&sort&direction&cursor&limit=50`. **5.D.2 Ficha** `/dashboard/contacts/:id`: panel izquierdo de identidad (externalId/dni/cuit/email/phoneE164/phone con dual display si phone≠phoneE164) + metadata (creado/actualizado/teamId) + atributos JSON pretty-printed con scroll; panel derecho timeline con `ToggleButtonGroup` (todo|email|wapi|audit) + lista de items renderizados con `Avatar` color-coded por kind (success.main para opens/clicks/read, error.main para failed/bounced, info.main para email genérico, success.dark para wapi genérico) + 10 iconos distintos (OpenInBrowser/Mouse/Email/Send/Done/DoneAll/ErrorOutline/Inbox/CallMade/History) + label localizado (kindLabel) + metadata expandido por kind (campaignName+subject+targetUrl+error en email.*, type+mediaCaption en wapi.message.*, campaignName+error en wapi.{sent,delivered,read,failed}, action en audit) + cursor pagination. 404 al cargar contact → redirect a `/dashboard/contacts` con notify error. **5.D.3 Import** `/dashboard/contacts/import`: textarea multiline (8-16 rows monoespaciada) para paste **o** input file (.csv, accept text/csv,text/plain) con FileReader; parser CSV inline custom (`parseCsv`) que maneja `\r\n`, comillas dobles con escape `""`, comas dentro de quoted strings, multilínea dentro de quotes; auto-detect mapping por header (ES/EN: externalId/dni/cuit/email/phone/firstName/lastName) en `autoMapping()`; UI mapping con Chip (header monospace) + Select de 10 opciones (8 fields + __attributes para guardar como custom attribute + __skip), las 8 keys fuertes se deshabilitan en otros selects una vez asignadas (excepto __attributes/__skip que pueden repetirse); preview de hasta 10 filas en `Table stickyHeader` con max-height 360px; validación inline (al menos 1 identificador en STRONG_KEYS, max 10k filas, no vacío); botón "Importar N filas" con `CircularProgress` durante submit; muestra resultado con Chip color-coded (status DONE/FAILED + total/processed/created/updated/suggested) + lista top-5 de errores + link a `/contacts/merge` si suggested>0. **5.D.X Merge UI** `/dashboard/contacts/merge`: filtro Select (PENDING|ACCEPTED|REJECTED) + lista de cards. Cada card muestra Chip con `EMAIL: <value>` o `Teléfono: <value>` + dos paneles lado-a-lado responsive (Stack column en xs, row en md+); `ContactPanel` left con border primary.main, right con divider; cada panel: Avatar+nombre+email/phone/external/dni/cuit en MiniRow + IconButton OpenInNew→target=_blank a `/contacts/<id>`; botones Aceptar (Confirm destructive: "se va a fusionar el de la derecha en el de la izquierda — los identificadores no-null del derecho rellenarán huecos del izquierdo, y el derecho se eliminará. Esta acción no se puede deshacer") + Rechazar (Confirm simple); detección **client-side** de strong-key conflict (externalId/dni/cuit distintos no-null entre left y right) que deshabilita Aceptar y muestra Alert warning explicando — backend igual valida y throw 400. Optimistic UI: al aceptar/rechazar, item desaparece de la lista local sin re-fetch. Cursor pagination. **Routing**: `App.tsx` agrega 4 rutas (contacts, contacts/import, contacts/merge, contacts/:id — orden importa para el matching de Nest-router; el `:id` queda al final). **Sidebar**: entry "Contactos" en grupo "Datos" (ContactsIcon, antes estaba `disabled:true`). Frontend typecheck ✅. **Sin tests** — el frontend del proyecto no tiene infra de tests aún; lógica crítica (search/merge/timeline/import) está cubierta backend-side con 598 tests. **Bulk actions (tag/untag/delete/export) deferidas a V2** — requerirían endpoints backend `POST /contacts/bulk/{tag,untag,delete}` que aún no existen. Próximo: smoke test E2E manual (cargar CSV con duplicados → verificar suggested + accept en /merge → ver el contact unificado en la lista) o avanzar 5.E (reportes consolidados). Sesión 50 anterior — **5.C Búsqueda y filtros avanzados**. Backend: nuevo método `ContactsService.search(params)` + endpoint `GET /api/contacts/search` registrado **antes** de `:id` y `by-identity` ya existente (Nest matchea por orden de declaración para rutas con segmentos estáticos vs `:param`). DTO `SearchContactsQueryDto` con: `cursor` (id de contact), `limit` (1-200, default 50), `q` (max 120), `tags[]` (max 50 ids, max 64 chars c/u, `@Transform` que acepta tanto array nativo como `tags=t1,t2` CSV string), `channel` (`email|wapi`), `hasOpened/Clicked/Bounced` (`@Transform` string→bool: acepta `true|1|false|0` además de bool nativo), `sort` (`createdAt|updatedAt|name`), `direction` (`asc|desc`). `buildSearchWhere()` arma where con: (a) `q` → mismo `OR` ILIKE que `list()` sobre firstName/lastName/email/externalId/phoneE164; (b) `tags` → `tags: { some: { tagId: { in: [...] } } }` (junction `ContactTag` con PK `(contactId,tagId)`, exista al menos un row con tagId en la lista); (c) `channel='email'` → `emailIdentities: { some: {} }` (Contact tiene al menos una `EmailContact` linkeada); (d) `channel='wapi'` → `wapiIdentities: { some: {} }`; (e) `hasOpened/Clicked/Bounced` (cualquiera true) → **reescribe** `emailIdentities` a `{ some: { reports: { some: filters } } }` con `firstOpenedAt: { not: null }` / `firstClickedAt: { not: null }` / `status: 'BOUNCED'` (puede combinar varios) — esto pisa `channel='email'` sin perder semántica porque ya implica que el contact tiene EmailContact + report con ese flag. **Sort**: `sortField=updatedAt|createdAt` → `[{<field>:dir},{id:dir}]`; `sortField=name` → `[{lastName:dir},{firstName:dir},{id:dir}]` para tiebreak estable. Default `updatedAt desc`. **Cursor**: `{ cursor: { id }, skip: 1 }` (usa el `@id String @id` del Contact, NO timestamp; trade-off: cursor estable pero requiere conocer el último id; el frontend ya tiene este patrón en `list()`). **Tests +12** en `contacts.service.spec.ts` (ahora 32 tests): q OR de 5 campos, tags `some.tagId.in`, channel=email/wapi, hasOpened/Clicked/Bounced individuales, sort=createdAt asc, sort=name (lastName+firstName+id), default sort updatedAt desc, cursor + nextCursor, combinación q+tags+channel+sort, hasOpened con channel=email se reescribe sin perder reports filter. **Deferido a V2**: filtros `lastActivityFrom/To` y sort `lastActivity desc`. Implementarlos correctamente requiere denormalizar `Contact.lastActivityAt` con backfill SQL (`MAX` de EmailReport.sentAt/EmailEvent.occurredAt/WapiReport.sentAt/WapiMessage.timestamp) + bumps en cada productor (EmailWorker:162/183/233/257, EmailCampaignsService, SesWebhookService, TrackService, WapiWorker, WapiInboxService, WapiWebhookService) — demasiado scope para V1 y nadie lo necesita aún. Documentado en MIGRATION_PLAN para Fase 5.E o futuro. **`tsvector`/full-text no V1**: ILIKE OR sobre 5 campos alcanza para volumen actual; migrar a tsvector queda para cuando haya datos en producción. 598/598 backend tests verde (+13 vs 585 — 12 nuevos en search + 1 que ya estaba). Backend typecheck ✅. Próximo: 5.D — frontend Contacts (lista + ficha + timeline + merge UI). Sesión 49 anterior — **5.B Timeline aggregator cross-canal**. Backend: nuevo `apps/backend/src/modules/contacts/contact-timeline.{service,dto,service.spec}.ts`. Endpoint `GET /api/contacts/:id/timeline?cursor=ISO&limit=&channel=email|wapi|audit` montado sobre `ContactsController` (método `getTimeline`, gate `read Contact`). **Service** `ContactTimelineService.getTimeline(contactId, query)` agrega 5 fuentes: (1) `EmailReport` (filtra por `EmailContact.contactId IN [...]` precalculado, emite 1 item @ `sentAt ?? createdAt` con kind `email.{status.lowerCase}` mapeado por `EMAIL_STATUS_KIND` const → queued/sent/failed/bounced/complained/suppressed/canceled, metadata con campaignId/campaignName/subject/error); (2) `EmailEvent` (filtra por `reportId IN [...]` que se obtuvieron en paso 1, emite 1 item @ `occurredAt` con kind `email.opened|clicked`, metadata con targetUrl/targetDomain/ip/deviceFamily); (3) `WapiReport` (filtra por `WapiContact.contactId`, **expande hasta 4 items por report** — uno por cada timestamp no-null entre `failedAt/readAt/deliveredAt/sentAt` con id `wapi.report.{rid}.{event}` para evitar colisiones en el sort, metadata con campaignId/campaignName/error); (4) `WapiMessage` (resuelve phones desde `contact.phoneE164` + `wapiContacts[].phone`, busca `WapiConversation` con `phone IN [...]`, después messages por `conversationId IN [...]`; emite 1 item @ `timestamp` con kind `wapi.message.{in|out}` según `fromMe`, metadata con conversationId/type/status/mediaMime/mediaCaption); (5) `AuditLog` (filtra por `resourceType='Contact' AND resourceId=contactId`, emite kind `audit` con metadata.action). **Cursor**: ISO timestamp opcional; cada fuente filtra `at <= cursorDate` (composto para EmailReport: `OR sentAt<=cursor / sentAt=null+createdAt<=cursor`); validación `Number.isNaN(date)` → 400. **Pagination**: buffer `PER_SOURCE_BUFFER=200` por fuente (heurística — evita una segunda round-trip cuando hay miles de eventos en una sola), merge in-memory, sort desc por `at` con tiebreak por `id` (string desc para estabilidad), slice `limit` (default 50, max 100), `nextCursor = sliced.last.at.toISOString()` si `items.length > limit`. **`channel`** filter: si está, salta las llamadas a las otras fuentes (verifica con tests). **DTO** `GetTimelineQueryDto` con `cursor IsISO8601`, `limit IsInt [1,100]`, `channel IsIn ['email','wapi','audit']`. **Lookup inicial** `prisma.scoped.contact.findFirst({id, select: {id,email,phoneE164,phone}})` → 404 si no existe (org-scope auto-aplicado). **Tests +7**: NotFound, cursor inválido, email reports + events ordenados desc, WapiReport expandido a 4 entries (con failedAt null → no emite), channel=audit no llama queries de email/wapi, limit + nextCursor, cursor descarta items posteriores. **Limitaciones V1 conocidas**: (a) cursor por timestamp solo — pierde precisión cuando varios items comparten `at` (mejorable a `(at,id)` en V2); (b) WapiMessage matchea sólo por `phone` exacto sin normalizar formatos divergentes (E.164 vs digits); (c) los modelos WAPI/Email son tenant-scoped, así que la timeline ve solo eventos del team del usuario actual (cross-team requeriría bypass del prisma extension). 585/585 backend tests verde (+7 vs 578). Backend typecheck ✅. Próximo: 5.C — búsqueda y filtros avanzados, o saltar a 5.D frontend (Contacts + Timeline + Merge UI). Sesión 48 anterior — **5.A.4 Merge backend: accept/reject + relink**. Backend: nuevo `apps/backend/src/modules/contacts/contact-merge.{service,controller,dto,service.spec}.ts`. Endpoints `GET /api/contacts/merge-suggestions?status&cursor&limit` (default `status=PENDING`, cursor pagination con `take=limit+1`, default 25 / max 100, `include` de left/rightContact con snapshot completo de identidad/profile), `POST :id/accept`, `POST :id/reject`. **`accept()`**: `findFirst` con include de ambos contacts; throw 404 si no existe, 400 si no está `PENDING`. `detectStrongKeyConflicts(left,right)` chequea externalId/dni/cuit distintos no-null → 400 (no se puede mergear si los IDs fuertes difieren — significa que NO son la misma persona). `buildProfilePatch(left,right)` arma un objeto con los campos de right que están null en left (externalId/dni/cuit/email/phoneE164/phone/firstName/lastName/attributes) — left gana donde tiene valor, right rellena los huecos. Todo el merge corre dentro de `prisma.scoped.$transaction(async (tx) => {...})`: (1) update del profile patch en left si hay cambios; (2) `EmailContact.updateMany` + `WapiContact.updateMany` por contactId (FK simple, sin uniqueness, paralelizado con Promise.all); (3) `ContactTag` con PK compuesto `(contactId,tagId)` requiere dedupe — `findMany` de leftTagIds, `deleteMany` de rightTags con tagId IN leftTagIds, después `updateMany` del resto a left; (4) mismo patrón para `ContactListMember` con PK `(listId,contactId)`; (5) `update` de la suggestion a `status=ACCEPTED` + `decidedByUserId=ctx.userId` + `decidedAt=now`; (6) `contact.delete` del right (cascade dispara y limpia las otras suggestions involucrando right). Devuelve `{ mergedContactId, removedContactId, relinked: { emailContacts, wapiContacts, contactTags, contactListMembers } }`. **`reject()`**: simple update de status a REJECTED + decidedBy/At, mismas validaciones de existencia y estado. `@Audit({ action: 'contact.merge.{accepted,rejected}', resourceType: 'ContactMergeSuggestion', resourceIdFrom: 'param:id', includeBody: false })` en ambos endpoints. CASL `ability.ts`: MEMBER ahora `can(['read','update'], 'ContactMergeSuggestion', { organizationId })`, VIEWER `can('read', ...)` (OWNER/ADMIN ya tenían `manage`). Tests +8 (list con status PENDING + cursor; accept happy path verifica patch + updateMany counts + suggestion ACCEPTED + delete right; accept con dedupe de tags y lists ya presentes en left; accept rechaza con BadRequest si dni distinto; accept NotFound; accept ya decidida → 400; reject happy + NotFound). 578/578 backend verde (+8 vs 570). 14/14 permissions verde. Backend typecheck ✅. **Frontend pendiente en 5.D**. Próximo: 5.B — Timeline aggregator. Sesión 47 anterior — **5.A.3 CSV import wizard inline + merge suggestions**. Backend: nuevo módulo `apps/backend/src/modules/contacts/contact-imports.{service,controller,dto,service.spec}.ts`. **Decisión de scope V1**: inline síncrono — el frontend parsea el CSV + aplica el mapping client-side y postea un JSON con `{fileName, fileSize, mapping, options?, rows[]}` (max 10k rows, max 100MB). Sin multer, sin BullMQ, sin worker. `ContactImportsService.create()` crea un `ContactImportJob` con `status='PROCESSING' + total + startedAt`, llama a `processRows()` y al finalizar update a `DONE` con counters/errors o a `FAILED` si throw. `processRows()`: bucle per-row con try/catch + `finally { processed++ }` (single-source-of-truth para no doble-contar), `errors[]` acumula `{index, message, row}`. `applyRow()`: (1) `findByStrongKey` (cascada externalId→dni→cuit con findFirst sucesivos) → si match, `updateContact(skipStrongConflicts:true)` (P2002 cae a un update sin las strong keys del row para no romper) + `maybeSuggestWeakConflict` (si row trae email/phone distintos, busca otros contacts en la org con esa weak key y crea suggestion EMAIL/PHONE) → counter `updated`. (2) Sino `findByWeakKey` (email→phoneE164) → si match: si row no trae strong key, `update` directo → `updated`; si trae strong key, `tryUpdateContact` (update con catch P2002 → false) → si OK `updated`, si choca `createContact` + `upsertSuggestion(weakMatch, newContact)` con `matchType=row.email?'EMAIL':'PHONE'` → `suggested`. (3) Sino `createContact` → `created`. `upsertSuggestion()` ordena `(left.id < right.id)` para que la unique `(left, right, matchType)` sea estable y swallowea P2002 (otra fila ya creó la suggestion). `normalizeRow()` reusa `identity.ts` (DNI/CUIT con throw si formato inválido — la fila falla pero el job sigue, errors[]). Validación "al menos un identificador" (`hasAnyIdentifier`): row sin externalId/dni/cuit/email/phoneE164 → error en `errors[]`, no toca DB, `processed++`. Endpoints `GET /api/contacts/imports` (cursor pagination default 25, max 100), `GET /api/contacts/imports/:id`, `POST /api/contacts/imports` con `@Audit({action:'contact.import.created', resourceIdFrom:'response:id', includeBody:false})`. Gates CASL: `read/create ContactImportJob`. Return types explícitos `ContactImportJobDto`/`ContactImportJobPage` para evitar leak de `Prisma.JsonValue` (TS2742). `ContactImportsController` registrado **antes** de `ContactsController` en `contacts.module.ts` para que `/contacts/imports` matchee antes que `/contacts/:id`. Tests: 8 specs nuevos con mocks de `prisma.scoped.{contact,contactImportJob,contactMergeSuggestion}` (created/strong-update/weak-no-strong/weak+P2002→suggestion/sin-identificadores→error/DNI-inválido-sigue/list-cursor/findOne-NotFound). Bug fix durante write: `processed++` antes del `continue` doblaba el contador (el `finally` ya lo incrementa). 570/570 backend tests verde (+8 vs 562). Backend typecheck ✅. Próximo: 5.A.4 — merge UI (endpoints accept/reject + frontend en 5.D). Sesión 46 anterior — **5.A.2 ContactsService org-scope + cascada de identidad**. Backend: nuevo util `apps/backend/src/modules/contacts/identity.ts` con `normalizeDni` (7-8 dígitos, limpia separadores), `normalizeCuit` (checksum mod-11 AR con weights `[5,4,3,2,7,6,5,4,3,2]`), `normalizePhoneE164` (`+<digits>`, 8-15 dígitos, rechaza `0` después de `+`), `normalizeEmail` (lowercase + trim), `normalizeExternalId` (trim, conserva case). Tests util: 19/19 verde. `ContactsService` reescrito org-scope: `list({cursor, limit, q, externalId, dni, cuit, email, phone})` con cursor pagination (`take=limit+1` trick, default 50, max 200), `q` arma `OR` insensitive sobre `firstName/lastName/email/externalId/phoneE164`, filtros normalizados antes del where; `findByIdentity({...})` cascada `externalId > dni > cuit > email > phoneE164` con cortocircuito al primer hit; `create/update` normalizan + `BadRequestException` si DNI/CUIT inválido + `ConflictException` (P2002) si strong key duplicado en org. `requireContext()` exige TenantContext, scope auto-inyecta `organizationId` via Prisma extension. Controller: endpoints `GET /api/contacts` (paginado), `GET /api/contacts/by-identity` (resolución), `GET/POST/PATCH/DELETE /api/contacts/:id`. `@Audit` en POST (`contact.created`), PATCH (`contact.updated`), DELETE (`contact.deleted`) con `resourceIdFrom: response:id` o `param:id`. DTOs nuevos: `CreateContactDto/UpdateContactDto` con `externalId/dni/cuit/email/phone/phoneE164/firstName/lastName/attributes` (todos `IsOptional` excepto requirement de "al menos uno"); `ListContactsQueryDto` (cursor, limit clamp `[1,200]`, q, filtros); `FindByIdentityQueryDto`. Spec rewrite: 13 tests (access control + list paginación + clamp + cursor + q + normalize + findByIdentity cascada + create normalize/conflict + update partial + remove). 562/562 backend verde (+32). Backend typecheck ✅ (fix `tenant-isolation.spec.ts`: `findAll()` → `list({})`). Frontend typecheck ✅ (no consumers). Sesión 45 anterior — **5.A.1 Contacts unificados: schema + migración + backfill**. Schema Prisma: `Contact` deja de estar team-scoped y pasa a **org-scope** con `teamId String?` (soft-ownership). Identity fields nuevos: `externalId String?` (clave de negocio del cliente, viene del CSV), `dni String?`, `cuit String?` (sin cifrar — pragmático), `phoneE164 String?` (normalizado `+<digits>`); el `phone` legacy queda durante la migración. Uniques compuestos por org en strong keys: `@@unique([organizationId, externalId])`, `@@unique([organizationId, dni])`, `@@unique([organizationId, cuit])` (nullable, postgres NULL-distinct). Índices en weak keys: `@@index([organizationId, email])`, `@@index([organizationId, phoneE164])`. `EmailContact` y `WapiContact` ganan `contactId String?` con FK `ON DELETE SET NULL`. Dos tablas nuevas: `ContactMergeSuggestion` (`leftContactId`/`rightContactId`/`matchType` enum `EMAIL|PHONE`/`matchValue`/`status` enum `PENDING|ACCEPTED|REJECTED`/`decidedByUserId?`/`decidedAt?`, unique `(left, right, matchType)`) para queue de fusiones por weak key, y `ContactImportJob` (`fileName`/`fileSize`/`status` enum `PENDING|PROCESSING|DONE|FAILED|CANCELLED`/`mapping JSON`/`options JSON?`/counters `total/processed/created/updated/suggested`/`errors JSON?`/`startedAt/finishedAt`) para tracking de imports CSV. Migración `20260514100000_contacts_unification` aplicada vía psql directo (drift previa) + INSERT manual en `_prisma_migrations`. Backfill idempotente en SQL: (1) `phoneE164` derivado en Contact existentes con regex `[^0-9]` y `+` prefix si ≥8 dígitos; (2) link `EmailContact.contactId` por `LOWER(TRIM(email))` con tie-break "longest name wins" via `DISTINCT ON ... ORDER BY LENGTH(name) DESC`; (3) crear Contact por cada `(orgId, normalized_email)` no linkeado, con `gen_random_uuid()::text` como id; (4-5) mismo patrón para WapiContact por phoneE164. Resultado en dev: 125 EmailContacts → 2 contacts únicos, 48 WapiContacts → 1 contact único (test data tenía sólo 2 emails + 1 phone distintos). `tenant-models.ts`: `Contact`/`ContactMergeSuggestion`/`ContactImportJob` movidos de `TENANT_SCOPED_MODELS` a `ORG_SCOPED_MODELS` (Tag/ContactList siguen team-scoped). CASL: subjects + 2 nuevos (`ContactMergeSuggestion`, `ContactImportJob`); OWNER/ADMIN ganan `manage` sobre los 3 con condición `organizationId`; MEMBER hace CRUD `Contact` org-scope + `create/read ContactImportJob` org-scope (`ContactList`/`Tag` siguen team-scope); VIEWER read-only sobre `Contact` y `ContactImportJob`. **Política de unificación**: cascada `externalId > dni > cuit > email > phoneE164`; strong keys (3 primeras) auto-mergean; weak keys (email/phone) crean `ContactMergeSuggestion` para resolución manual. **DNI/CUIT sin cifrar** por decisión explícita del dueño (pragmático). Próximo paso: 5.A.2 — `ContactsService` org-scope + endpoints `GET/POST/PATCH/DELETE /api/contacts`. Sesión 44 anterior — **4.S.3 Audit Log endpoint + frontend (Stage 6)**. Cierra el bloque 4.S completo. Backend: nuevo módulo `apps/backend/src/modules/audit-logs/` (service + controller + DTO + spec) que expone `GET /api/audit-logs` con cursor pagination (`take=limit+1` trick, default 50, max 200) y filtros opcionales `actorUserId`/`resourceType`/`resourceId`/`action`/`from`/`to`. Tenant-scoped via `prisma.scoped.auditLog` (org-scope, sin teamId). Enriquecimiento de actor en una sola query batch a `User.findMany({ where: { id: in: actorIds } })` y mapeo en memoria — devuelve `{ name, email, avatarUrl }` o null si fue una acción de scheduler/sistema o si el user fue borrado. Permission gate CASL: agregado `'AuditLog'` al `SubjectName`, `OWNER`/`ADMIN` ganan `can('read', 'AuditLog', { organizationId })`. Tests +9 (vacío, paginación con nextCursor, clamp limit `[1,200]`, cursor con skip:1, filtros combinados, fechas from/to, enrich actor, sin actor, actor borrado). 539/539 backend tests verde. Frontend: nueva ruta `/dashboard/audit` (`AuditLogPage.tsx`) con header + Paper de filtros (Grid 6 columnas: actor user ID, acción, resourceType, resourceId, datetime-local from/to, botones Aplicar/Limpiar/Refresh) + Paper con tabla (fecha, actor con avatar+nombre+email o chip "sistema", acción como chip monoespaciado, recurso type+id, IP). Click en fila → `Drawer` derecho con detalle completo de campos + bloque `<pre>` con metadata pretty-printed JSON, scrollable, monospace, theme-aware (action.hover bg). Botón "Cargar más" con cursor. Sidebar entry "Audit log" (HistoryIcon) en grupo "Cuenta". Frontend typecheck ✅. Sesión 43 anterior — **4.S.2 Audit Log: cobertura ampliada (Stages 3-5)**. Sigue sobre la infra de 4.S.1. Stage 3 — WAPI: `WapiBotController` 5 endpoints (bot.updated/mediaUploaded/draftSaved/published/draftDiscarded; flow payloads con includeBody:false), `WapiInboxController` 8 endpoints (conversation.messageSent/mediaSent/taken/assigned/unassigned/resolved/reopened/held; mediaSent con includeBody:false), `WapiQuickRepliesController` 3 endpoints (created/updated/deleted), `WapiTemplatesController` 5 endpoints (syncedFromMeta/submittedToMeta/created/updated/deleted). Sandbox/step queda sin auditar (preview in-memory). Sin opt-out controller (procesa internamente desde inbound). Stage 4 — Email: `EmailCampaignsController` 8 endpoints (mismo set que WAPI), `EmailCampaignSchedulerService` con log manual post-send (`actorUserId:null, metadata:{source:'scheduler', name}`) dentro del TenantContext.run, `SmtpAccountsController` 5 endpoints (created/verified/testSent/updated/deleted), `EmailTemplatesController` 3 endpoints (created/updated/deleted; create/update con includeBody:false por payloads HTML/JSON Unlayer), `SuppressionsController` 3 endpoints (unsubscribeAdded/unsubscribeRemoved/bounceRemoved). Stage 5 — org-level: `OrganizationsController` 1 endpoint (org.webhookSlugRegenerated), `TeamsController` 3 endpoints (created/updated/deleted), `TeamMembersController` 3 endpoints (memberAdded/memberRoleChanged/memberRemoved). Tests: `email-campaign-scheduler.service.spec.ts` actualizado por nueva dep AuditLogService. 232/232 verde en suite combinada. Backend typecheck ✅. **Stage 6 (frontend `/dashboard/audit` con tabla + filtros) queda como próximo commit.** Sesión 43 anterior — **4.S.1 Audit Log de transacciones (Stages 1-2)**. Backend: nuevo módulo global `apps/backend/src/common/audit/` con `AuditLogService.log()` cross-tenant fire-and-forget (toma actorUserId/orgId/teamId del `TenantContext` con override para jobs, descarta si no hay org, sanitiza recursivamente keys sensibles via regex `/access[_-]?token|app[_-]?secret|verify[_-]?token|password|secret|api[_-]?key|enc$/i` → `[REDACTED]`, try/catch interno que nunca propaga); decorator `@Audit({ action, resourceType?, resourceIdFrom?, includeBody? })` con sources `param:<key>|body:<key>|response:<key>`; `AuditInterceptor` registrado como `APP_INTERCEPTOR` global, lee metadata vía `Reflector`, captura body/params/ip(con first-hop x-forwarded-for)/userAgent, escribe vía `tap()` rxjs sólo on-success. Migration `20260513100000_audit_log_resource_actor_indexes` con índices `(orgId, resourceType, resourceId)` y `(actorUserId, createdAt)`. Cobertura Stage 2: `WapiCampaignsController` 8 endpoints (created/updated/contactsAdded/sent/paused/resumed/forceClosed/deleted, addContacts con includeBody:false), `DevSimulatorController` 5 endpoints (inbound.text/media/reaction/button + status, media con includeBody:false), `WapiConfigsController` 4 endpoints (secretsRevealed/created/updated/deleted), `WapiCampaignSchedulerService` con llamada manual post-send (`actorUserId:null, metadata:{source:'scheduler', name}`) dentro del `TenantContext.run`. Tests +13 (8 service + 5 interceptor) + actualización scheduler.spec por nueva dep. 17/17 verde en suite audit+scheduler. Backend typecheck ✅. Stages 3-6 (WAPI inbox/bot/quick-replies, Email/SMTP, org-level, frontend `/dashboard/audit`) quedan para próximos commits. Sesión 42 anterior — **4.R Scheduler de campañas (WAPI + Email)**. Backend: dos workers cross-tenant con `setInterval(60s)` — `WapiCampaignSchedulerService` y `EmailCampaignSchedulerService` — que leen filas con `status='SCHEDULED' AND scheduledAt <= NOW()` (batch 50) y disparan `send()` per-row bajo `TenantContext` sintético construido con `organizationId/teamId` de la fila. Multi-instance safe via la transición a `PROCESSING` en transacción dentro de `send()`. Mismo patrón que `WapiBotWaitingExpirerService` (setInterval simple, `unref()`, no usa `@nestjs/schedule`). **Fix bug en `update()`** — DRAFT + scheduledAt → SCHEDULED, SCHEDULED + scheduledAt:null → DRAFT, PAUSED no se toca. Aplicado en ambos services. **Antes**: `scheduledAt` era decorativo, ningún worker lo monitoreaba; el botón "Enviar" mandaba al instante; tab "Programadas" siempre vacío porque update no transicionaba el status. Tests +13 (7 schedulers happy/empty/resiliente/filtros + 6 transición update). 397/397 wapi+email verde, backend typecheck ✅. **También**: aplicadas localmente migrations `20260511100000_organization_webhook_slug` (4.P) y `20260512100000_wapi_config_send_delay` (4.Q) vía psql directo + INSERT en `_prisma_migrations` (additive con defaults, sin pérdida de data); `prisma migrate status` reporta "Database schema is up to date" con 21 migrations. Sesión 41 anterior — **4.Q Throttle configurable por línea + override per-campaña**. Schema: `WapiConfig.sendDelayMinMs/sendDelayMaxMs Int @default(30000/60000)` + migration `20260512100000_wapi_config_send_delay`. `WapiCampaign.config` (JSON) ahora acepta keys opcionales `delayMinMs/delayMaxMs` con validación en `WapiCampaignsService.assertCampaignConfig()` (rango `[1000ms, 3_600_000ms]`, min ≤ max). Worker: `WapiWorkerService.jitterMs({campaignConfig, configRel})` resuelve cascada `campaign.config.delay* → WapiConfig.sendDelay* → WAPI_DELAY_MIN/MAX_MS env → defaults 30s/60s`, ordena lo/hi defensivamente. DTOs `Create/UpdateWapiConfigDto.sendDelayMinMs/Max` con `@IsInt @Min(1000) @Max(3_600_000)`; service `assertDelayRange()` cruza con valor persistido en updates parciales. Live snapshot extendido: `LiveCampaignSummary` agrega `delayMinMs/Max + delaySource ('campaign'|'config')`, `LiveConfigUsage` agrega `sendDelayMinMs/Max`. Frontend: dialog `WapiConfig` con sección "Velocidad de envío" (TextField min/max en segundos + helper en vivo `~X envíos/min · ~Y/hora`); `WapiCampaignDetailPage` con switch "Pisar velocidad de envío para esta campaña" + Collapse con TextFields min/max + estimación; `WapiLivePage` con tooltip de velocidad efectiva sobre el nombre de campaña + chip mini `Velocidad ★` cuando hay override + tooltip de delay base sobre nombre de línea. Tests: +5 worker (cascada + min>max sucio), +4 configs service (range + partial update), +5 campaigns service (assertCampaignConfig), live spec actualizado con delaySource/sendDelayMin*. Backend typecheck ✅, frontend typecheck ✅. Migración pendiente de aplicar localmente (drift previa). Sesión 40 anterior — **4.P Webhook URL org-scoped**. Schema `Organization.webhookSlug String @unique` (formato `wbh_<22-24 chars>`) + migration `20260511100000_organization_webhook_slug` con backfill `wbh_<md5>` para orgs existentes; nuevas orgs reciben slug desde `crypto.randomBytes(18).toString('base64url')` en `ClerkWebhookService.handleOrganizationCreated`. Nuevo `OrganizationsModule` con endpoint `POST /api/orgs/me/webhook-slug/regenerate` (gate `manage Organization`). `WapiWebhookController` migrado de URL única global a rutas `:slug` (`GET/POST /api/webhooks/wapi/:slug`) — verify y receive filtran `WapiConfig.organizationId = orgId`; cache slug→orgId in-memory TTL 60s; slug inexistente → 404 sin info-leak. Endpoint `GET /api/wapi/configs/:id/reveal-secrets` (gate `manage Organization`) que devuelve `{ webhookVerifyToken }` en claro con log WARN para auditoría. `MeOrganization.webhookSlug` surface en `/api/me/context`. Frontend `WapiConfigsPage`: card top-level con la URL completa (`{API_BASE_URL}/api/webhooks/wapi/{slug}`) + botón copiar + botón **Regenerar URL** (gated OWNER/ADMIN, confirm destructive); por fila botón llave para revelar/ocultar verify token + botón copiar. Tests `wapi-webhook.controller.spec.ts` reescritos: 16/16 verde — scoping por slug, cache (segunda llamada no re-consulta organization), multi-config misma org, slug inexistente → 404, firma inválida → 403, dev sin appSecret, payloads ignorados. Backend typecheck ✅ + frontend typecheck ✅. **No mantenemos URL legacy**: la ruta antigua se reemplaza directo (estamos pre-launch). Migración pendiente de aplicar localmente: `prisma migrate reset` lo aplicaría junto a otras drift (decidir cuándo). Sesión 39 anterior — **4.J Live Dashboard WAPI**. Backend: nuevo módulo `wapi/live` con `WapiLiveService.snapshot()` que paraleliza tres recolectores: `collectCampaigns(since5min)` (findMany top 25 con status PROCESSING/PAUSED + dos `groupBy` paralelos sobre WapiReport para totales por (campaignId,status) y throughput 5min), `collectConfigs(since24h)` (findMany configs activas + groupBy SENT 24h + segundo findMany para mapear campaignId→configId — Prisma groupBy no agrupa por relaciones), `collectInbox()` (3 counts paralelos UNASSIGNED+escalated, WAITING, escalated total + findFirst de la más antigua sin asignar). Endpoint `GET /api/wapi/live/snapshot` con guards Clerk+Tenant+Policies(read Campaign). Spec con 4 tests (478/478 verde). Fix typing groupBy: separar awaits y castear el resultado, no la promise — el `as Promise<…>` dentro de `Promise.all` rompe la inferencia de overload. Frontend: nueva página `/dashboard/wapi/live` con 3 widgets en stack (Campañas en curso con tabla + funnel chips + LinearProgress + throughput; Uso de líneas con barras color por umbral 80/100% y badge TEST; Inbox snapshot con 3 KPI cards + edad de la más antigua + link a inbox); chip "● En vivo" con FiberManualRecordIcon (verde/gris según `socket.connected`); re-fetch debounced 500ms ante eventos `wapi.report.updated`/`wapi.report.log`/`wapi.conversation.updated` con coalesce vía `inFlightRef+pendingRef` para no apilar requests; `liveApi.snapshot(api)` + types mirror del backend; sidebar entry "Dashboard live" (MonitorHeartIcon) primero del grupo WhatsApp; ruta `wapi/live` en App.tsx. Frontend typecheck ✅ limpio. Sesión 38 anterior — **4.O.6 Suspensión bot + estado WAITING**. Backend: schema Prisma extendido con `escalated`/`botSuspended`/`waitingUntil`/`lastAssignedUserId` en `WapiConversation` + `botWaitingTtlMin` en `WapiConfig` + valor `WAITING` en enum `WapiConversationStatus`; engine guard inicial (`if botSuspended return`) + acción HANDOFF marca `escalated:true, botSuspended:true` y reabre si estaba RESOLVED; webhook detecta WAITING → vuelve a UNASSIGNED al recibir mensaje del cliente; button INBOX setea `priority+escalated+botSuspended`; inbox service filtra siempre por `escalated=true` (uniforme cross-rol — admin scope verificado), `mine` retorna OR(ASSIGNED-mías + WAITING-lastMine), `assign/take` setean `botSuspended+escalated` y limpian `waitingUntil`, `resolve` apaga `botSuspended` y `waitingUntil` (mantiene escalated), `reopen` reactiva `botSuspended`; nuevo endpoint `POST /api/wapi/inbox/conversations/:id/hold` → `putOnHold` calcula `waitingUntil = now + cfg.botWaitingTtlMin*60000` y emite socket; nuevo worker `WapiBotWaitingExpirerService` (setInterval 5min, cross-tenant, findMany+update individuales, `unref()` para no bloquear procesos) que devuelve WAITING vencidas a UNASSIGNED. 474/474 tests verdes (5 nuevos en inbox, 4 nuevos en expirer, 1 fix en me.service.spec por ConfigService dep). Frontend: `WapiConversationStatus` con WAITING + `waitingUntil`/`lastAssignedUserId` en list/detail/event types + `inboxApi.hold()`; `ConversationHeader` con botón "Poner en espera" (PauseCircleOutlineIcon) visible solo si isMine, `StatusChip` muestra WAITING con countdown vivo (`useCountdown` tick 30s) y chip "lo tenías vos" cuando `lastAssignedUserId===currentUserId`; `WapiInboxPage.handleHold` + `onUpdated` reducer propaga `waitingUntil`/`lastAssignedUserId`; `ConversationList` muestra chip "En espera" en filas WAITING; `WapiSimulatorChatPage` actualizado con `onHold`. Frontend typecheck ✅ (solo errores pre-existentes en bots/validateClient/RouterPanel). Migración aplicada: `20260510120000_wapi_bot_suspension_waiting`. Sesión 37 anterior — 4.O.5 Nodo SET_VAR.)
