...
- **Fase actual:** Fase 4 — Canal WhatsApp Cloud API (**sub-A ✅, sub-B ✅, sub-C ✅, sub-D ✅, sub-E ✅, sub-F.1 ✅, sub-F.1.a ✅, sub-F.1.b ✅, sub-F.1.c ✅ (mapping vars), sub-F.2.a ✅, sub-F.2.b ✅, sub-F.2.c ✅ (markdown + dark mode preview), sub-F.2.d ✅ (media upload Meta + storage local + render por tipo), sub-F.3 ✅ (inbox backend), sub-F.4 ✅ (inbox frontend), sub-G ✅ (quick replies admin), sub-L (MVP ✅ + ✅ chat ida-vuelta con `isTestMode`; pendiente virtual numbers + audit log), sub-L.1 ✅ (filtro inbox por línea)**; siguen **4.H opt-out, 4.I welcome, 4.J live dashboard, 4.K botones**)
- **Fases completadas:** Fase 0 ✅ + Fase 1 ✅ + Fase 2 ✅ + **Fase 3 ✅** (3.E inbound postergado, decisión del dueño)
- **Última actualización:** 2026-05-05 (cierre Sesión 28, 4.M chat simulado ida-vuelta + filtro inbox por línea)
- **Branch principal:** `main`
- **Próximo paso al volver**: ⚠️ **Antes que nada**: detener backend dev server, correr `pnpm -C packages/prisma prisma generate` (la regeneración falló durante la sesión por DLL locked) y reiniciar backend. Luego smoke test del **chat simulado** (`/dashboard/dev/wapi/chat`): 1) crear/editar una `WapiConfig` activando el toggle "Modo test"; 2) abrir el chat, pickear esa config + escribir un phone (cliente virtual); 3) enviar un texto desde el pane izq → debe aparecer la conversación en el pane der; 4) responder desde el inbox del pane der → debe aparecer en el pane izq como incoming; 5) repetir con media (img/doc); 6) probar mark resolved + reopen. Después: decidir entre **4.H (opt-out keywords)** o **4.J (live dashboard)**.
